...
| 17 | Control fields are **dataset-derived, not inferred**: `control_modality` (`joint`/`end-effector`) reads the action feature names; per-segment `active_dof` (`arm`/`gripper`/`both`) is a deterministic function of which action dims move beyond a threshold over the segment | `src/robolabel/control.py` + `tests/test_control.py`; enriched on pick-place/pour/fold (all `joint`; `pour water`→`arm`, `retract`→`arm`) | **verified (deterministic, no VLM)**; the `active_dof_threshold` (rubric.yaml) is a documented choice, not validated against human dof labels |
| 18 | Stores an optional **retrieved** subgoal — the same-phase end frame from a *different* episode — **alongside** the real keyframe (never replacing it), for copy-shortcut-free policy eval; robolabel **does not generate** images | `src/robolabel/retrieve.py` + `tests/test_retrieve.py`; `robolabel enrich --retrieve-subgoals` (pour 35/41, fold 35/39 matched) | **verified that the selection is a real same-phase frame from another episode** (deterministic/seeded; unmatched phases left null); **downstream training/eval utility is untested** |
