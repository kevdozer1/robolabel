...
| 25 | **Proprioception-fused grasp/release snap** (deterministic, no VLM): snaps a grasp/release-onset boundary to the nearest gripper open↔close transition within a small window; **no-ops where the gripper doesn't mark the boundary** (pour/fold). On the SO-101 gold it **did not improve** recall@±5 (0.174 → 0.130 at window 8) because that gold is S0-anchored, not gripper-aligned — so it is **OFF by default** (`segmentation.snap_contact`) | `src/robolabel/snap.py` + `tests/test_snap.py`; `scripts/eval_snap.py` vs `so101_gemini/gold.json`; `FRESH_TRIAL_REPORT.md` | **verified mechanics by test; verified-on-one-dataset that it does not help this (S0-anchored) gold** → off by default; available for gripper-aligned ground truth |
