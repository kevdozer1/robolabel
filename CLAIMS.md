...
| 19 | Deterministic episode **`speed`** metadata (mean per-step action velocity, binned fast/medium/slow vs the dataset) — one of pi0.7's two metadata signals, and the more informative one on easy datasets where quality is near-degenerate | `src/robolabel/speed.py` + `tests/test_modules.py` | **verified (deterministic, no VLM)**; binning is a relative-to-dataset choice; informativeness-for-training is unvalidated |
...
| 21 | **Curation**: a single `curation_value = f(quality, novelty)` (config weights) plus an optional value-tiered **overlay** (`full`/`reduced`/`minimal`, or top-cut `keep`/`cut`) that never deletes data — precedented by the Smart Black Box (value-tiered storage/compression) and "train on the most valuable ~20%" curation work | `src/robolabel/curation.py` + `tests/test_modules.py`; docs cite the precedents | **machinery sound + precedented; downstream training utility UNVALIDATED**; off by default |
