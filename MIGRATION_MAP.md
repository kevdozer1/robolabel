...
`labelkit` is a deliberate min-cut extraction from a private research monorepo (`annotation_pipeline` / `bridgeengine`). This file records every monorepo module that was **ported**, **adapted**, or **deliberately left behind**, with a one-line reason. It is maintained throughout the extraction; it is the eulogy for the complexity that did not make the cut.
...
src/labelkit/
  ...
  cli.py                # labelkit annotate|review|reliability|gate|export|cost|demo
  demo.py               # synthetic-episode generation + fully offline end-to-end demo
```

## Ported (transcribed and de-coupled)

| Monorepo source | labelkit target | One-line reason |
|---|---|---|
...
- All ported code is re-authored from a single-author private repo owned by the same author publishing `labelkit`; no third-party copied code is carried over. Dependencies (`pillow`, `pandas`, `pyarrow`, `requests`, `pyyaml`, `numpy`, `streamlit`, `lerobot`) are used through their public APIs, not vendored.
- License: Apache-2.0 (see `LICENSE`).
- **Open flag:** `lerobot` is GPL-touching in some optional extras; `labelkit` depends on it only through the public `LeRobotDataset` read API and pins the version in the README. Confirm the pinned `lerobot` license is compatible with redistribution before tagging 1.0.
